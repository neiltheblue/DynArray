 ...
 * For example:
 *
 * DEFINE_DYNARRAY_TYPE(long, dynArrayLong, dynArray, .capacity=100)
 *
 ...
 *
 * The final named argument '.capacity=100' will override the default capacity
 * value for generated dynamic arrays.
 *
 ...
 * @param ... the var args allow for overriding the dynamic array default
 * settings
 *
 */
#define DEFINE_DYNARRAY_TYPE(TYPE, STRUCT, NAME, ...)                          \
  ...
  void _getter_##NAME(const void *pDA, const size_t index, void *value) {      \
    *(TYPE *)value = *(((STRUCT *)pDA)->array + index);                        \
                                                                               ...
                                                                               \
  STRUCT *NAME() {                                                             \
    return _createDynArray(                                                    \
        (DynArrayParams){                                                      \
            .capacity = 10, .growth = 1.5, .size = 0, __VA_ARGS__},            \
        sizeof(STRUCT), sizeof(TYPE), _setter_##NAME, _getter_##NAME);         \
  }
